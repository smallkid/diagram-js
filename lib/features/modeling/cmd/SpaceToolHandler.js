...
SpaceToolHandler.prototype.preExecute = function(context) {
  var delta = context.delta,
      ...
      // TODO(@barmac): replace with new Map() once we drop PhantomJS
      oldBounds = new BoundsMap();
  ...
  forEach(resizingShapes, function(shape) {
    oldBounds.set(shape, getBounds(shape));
  });
  ...
  forEach(connections, function(connection) {
    ...
    if (includes(affectedShapes, source) && includes(affectedShapes, target)) {
      ...
      if (includes(movingShapes, source)) {
        ...
        layoutHints.connectionStart = getResizedSourceAnchor(connection, source, oldBounds.get(source));
      } else if (includes(resizingShapes, target)) {
        layoutHints.connectionEnd = getResizedTargetAnchor(connection, target, oldBounds.get(target));
      }
...
/**
 * Limited ES6 Map polyfill. Supports only `set` and `get` methods.
 */
function BoundsMap() {
  var entries = [];

  function getEntryByKey(key) {
    return find(entries, function(entry) {
      return key === entry[0];
    });
  }

  this.set = function(key, value) {
    var existingEntry = getEntryByKey(key);
  ...
  this.get = function(key) {
    var entry = find(entries, function(entry) {
      return key === entry[ 0 ];
    });
